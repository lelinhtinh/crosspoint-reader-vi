...
struct PngContext {
  ...
  PngContext()
      : renderer(nullptr), config(nullptr), screenWidth(0), screenHeight(0), scale(1.0f), srcWidth(0), srcHeight(0),
        dstWidth(0), dstHeight(0), lastDstY(-1), caching(false) {}
};
...
int pngDrawCallback(PNGDRAW *pDraw) {
  ...
  if (!ctx || !ctx->config || !ctx->renderer)
    return 0;
  ...
  if (dstY == ctx->lastDstY)
    return 1;
  ctx->lastDstY = dstY;

  // Check bounds
  if (dstY >= ctx->dstHeight)
    return 1;

  int outY = ctx->config->y + dstY;
  if (outY >= ctx->screenHeight)
    return 1;
  ...
  for (int dstX = 0; dstX < dstWidth; dstX++) {
    ...
    if (outX < screenWidth) {
      ...
      if (caching)
        ctx->cache.setPixel(outX, outY, ditheredGray);
    }
